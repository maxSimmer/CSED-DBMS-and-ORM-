...
# Assuming you have already defined your Base and classes (Student, Event, Attends)
# ...
...
# Define your classes and relationships here if not already defined

# Query to retrieve information about students who attended more than one event
result = (
    session.query(
        Student.sid.label('StudentID'),
        Student.sfirst.label('FirstName'),
        Student.slast.label('LastName'),
        func.count(func.distinct(Attends.eid)).label('numberOfEvents'),
        func.string_agg(Event.ename, ', ').label('AttendedEvents')
    )
    .join(Attends, Attends.sid == Student.sid)
    .join(Event, Event.eid == Attends.eid)
    .group_by(Student.sid, Student.sfirst, Student.slast)
    .having(func.count(func.distinct(Attends.eid)) > 1)
    .order_by(func.count(func.distinct(Attends.eid)).desc())
    .all()
)
...
for row in result:
    print(
        f"StudentID: {row.StudentID}, FirstName: {row.FirstName}, LastName: {row.LastName}, "
        f"numberOfEvents: {row.numberOfEvents}, AttendedEvents: {row.AttendedEvents}"
    )
